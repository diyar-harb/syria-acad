...
const StudentDashboard = () => {
  ...
  const handleSendMessage = e => {
    ...
    setMessages([...messages, { id: messages.length + 1, ...newMessage, date: new Date().toISOString().slice(0, 10) }])
      ...
      <div className="dashboard-content">
        ...
        <div className="dashboard-grid">
          <div className="recent-courses">
            <h2>المواد النشطة</h2>
            <SmartDataGrid rows={recentCourses} columns={courseColumns} loading={loading} pageSize={5} />
          </div>

          <div className="upcoming-exams">
            <h2>الاختبارات القادمة</h2>
            <SmartDataGrid rows={upcomingExams} columns={examColumns} loading={loading} pageSize={5} />
          </div>
        </div>
            ...
            {notifications.map(notification => (
              <div key={notification.id} className="notification-card">
                <div className="notification-header">
                  ...
                  <span className="notification-date">{new Date(notification.date).toLocaleDateString('ar-SA')}</span>
                </div>
        ...
        <div className="messages-section">
          ...
          <form className="send-message-form" onSubmit={handleSendMessage} style={{ marginBottom: '1rem' }}>
            <input type="text" name="subject" placeholder="المادة" value={newMessage.subject} onChange={handleMessageChange} style={{ marginLeft: 8 }} />
            <input type="text" name="teacher" placeholder="اسم المعلم" value={newMessage.teacher} onChange={handleMessageChange} style={{ marginLeft: 8 }} />
            <input type="text" name="content" placeholder="اكتب رسالتك..." value={newMessage.content} onChange={handleMessageChange} style={{ marginLeft: 8, width: 200 }} />
          ...
          <ul className="reports-list">
            {reports.map(report => (
              <li key={report.id}>
                ...
                <a href={report.file} download className="btn btn-secondary" style={{ marginRight: 8 }}>
                  تحميل
                </a>
              </li>
            ))}
